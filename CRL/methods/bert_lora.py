...
_CONFIG_FOR_DOC = "BertLoRAConfig"
...
class BertLoRAConfig(PretrainedConfig):
    r"""
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        if config.apply_lora:
            self.query = lora.Linear(config.hidden_size, self.all_head_size, r=config.lora_r)
        else:
        ...
        if config.apply_lora:
            self.value = lora.Linear(config.hidden_size, self.all_head_size, r=config.lora_r)
        else:
...
BERT_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`BertLoRAConfig`]): Model configuration class with all the parameters of the model.
...
class BertForMaskedLMLoRA(BertPreTrainedModel):
