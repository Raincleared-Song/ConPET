...
def load_vocab(file: str) -> Dict[str, int]:
...
class CPMBeeTokenizer(object):
    def __init__(
        ...
        self.encoder = load_vocab("bmt_models/bee_vocab.txt")
    ...
    def encode(
        self, text: str, past_table: Dict[int, str] = None
        ...
        for x in self.tokenize(text):
            if x.is_unk or (x.is_special and (x.token not in self.encoder)):
                ...
                ret.append(self.encoder[self.unk_token])
