...
class Attention(torch.nn.Module):
    def __init__(
        ...
        if lora_r > 0:
            self.project_q = lora.Linear(self.dim_model, self.num_heads * self.dim_head,
                                         bias=False, dtype=dtype, r=lora_r)
            # nn.init.kaiming_uniform_(self.project_q.lora_B, a=math.sqrt(5))
            self.project_k = lora.Linear(self.dim_model, self.num_heads * self.dim_head,
                                         bias=False, dtype=dtype, r=lora_r)
            # nn.init.kaiming_uniform_(self.project_k.lora_B, a=math.sqrt(5))
        else:
    ...
    def forward(
        ...
        h_q = self.project_q(hidden_q) / math.sqrt(self.dim_model)
        h_k = self.project_k(hidden_kv) / math.sqrt(self.dim_model)
        h_v = self.project_v(hidden_kv) / math.sqrt(self.dim_model)
        ...
        score = torch.masked_fill(
            score,
            torch.eq(attention_mask.view(batch_size, 1, len_q, len_k), False),
