...
def main():
    ...
    for idx in range(args.start, total_bound):
        ...
        scp_cmd = ['scp', f'a100node4:/data/private/songchenyang/continual_re_typer/checkpoints/{exp_path}/flag', './']
        print(' '.join(scp_cmd))
        ...
        scp_cmd = ['scp', '-r',
                   f'a100node4:/data/private/songchenyang/continual_re_typer/checkpoints/{exp_path}', './checkpoints/']
        print(' '.join(scp_cmd))
        subp = subprocess.Popen(scp_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        subp.wait()
        ...
        assert config_base['train']['continual_method'] == 'emr'
        assert config_base['train']['train_expert_selector'] == 0 \
               and not config_base['train']['use_expert_selector'] \
               and config_base['dataset']['batch_limit_policy'] == 0 \
               and (idx == 1 or config_base['dataset']['use_selected'])
        ...
        scp_cmd = ['scp', sample_file, f'a100node4:/data/private/songchenyang/continual_re_typer/cache/']
        print(' '.join(scp_cmd))
        subp = subprocess.Popen(scp_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        subp.wait()
        assert subp.returncode == 0
