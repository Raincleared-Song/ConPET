...
from kernel import init_tokenizer_model, init_optimizer, train_emar, train
...
def main():
    ...
    for idx in range(1, args.start):
        ...
        for key, sample in selected_samples['train_infer'] + selected_samples['valid_groups']:
            tag, text = sample
            proto_memory[tag].append((key, sample))
    ...
    for idx in range(args.start, total_bound + 1):
        ...
        # EMAR train_function
        replay_frequency = get_emr_replay_frequency(args.dataset_name, idx, len(mem_train_data) * 2)
